...
"""
`GraphTheory(draw_pkgs::Bool=false, verbose::Bool=true)` loads my packages for working on graphs
and sets the optmizer to be `Gurobi` (non-verbose). The list is generated by `list_of_graph_theory()`.

If `draw_pkgs` is set to true, also load the packages returned by  `list_of_graph_draw()`
"""
